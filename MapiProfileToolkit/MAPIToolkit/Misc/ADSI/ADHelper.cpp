...
std::wstring GetDCName()
{
	...
	if (pADsys)
	{
		...
		EC_HRES(pADsys->GetAnyDCName(&bstrDCName));
		if (bstrDCName)
		{
...
std::wstring LDAPSearchUserDN(std::wstring wszSearchAttributeName, std::wstring wszSearchValue, ULONG ulAdTimeout)
{
	...
	EC_HRES(ADsOpenObject(GetDCName().c_str(),
		NULL,
		NULL,
		ADS_SECURE_AUTHENTICATION,
		//Use Secure Authentication
	...
	EC_HRES(pObject->Get(L"defaultNamingContext", &var));

	wszAdsiPathName = L"LDAP://" + std::wstring(var.bstrVal);

	EC_HRES(ADsOpenObject(wszAdsiPathName.c_str(),
		NULL,
		NULL,
		ADS_SECURE_AUTHENTICATION,
		//Use Secure Authentication
	...
	EC_HRES(pConfigNC->SetSearchPreference(const_cast<ADS_SEARCHPREF_INFO *>(prefs), cPrefs));

	LPWSTR lpszAttrName = L"distinguishedName";
	int iCount = 0;
	DWORD x = 0L;

	//Return non-verbose list properties only
	EC_HRES(pConfigNC->ExecuteSearch((LPWSTR)wszSearchFilter.c_str(),
		&lpszAttrName,
		1,
		&hSearch));
	...
	//of data
	EC_HRES(pConfigNC->GetFirstRow(hSearch));
	...
	while (hRes != S_ADS_NOMORE_ROWS)
	{
		...
		{
			EC_HRES(pConfigNC->GetColumn(hSearch, pszColumn, &col));
